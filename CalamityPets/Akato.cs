...
namespace PetsOverhaulCalamityAddon.CalamityPets
{
    public sealed class AkatoEffect : PetEffect
    {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            {
                Pet.NewPetSourcedProjectile(PetUtils.GetSource_Pet(EntitySourcePetIDs.PetProjectile), target.Center, Vector2.Zero, ModContent.ProjectileType<PetExplosion>(), hit.SourceDamage * (explosionMult + StackExplosionBonus), 0, Player.whoAmI, explosionSize + StackExplosionSize, damageClass: hit.DamageType);
                if (ModContent.GetInstance<PetPersonalization>().AbilitySoundEnabled)
                {
    ...
    public sealed class ForgottenDragonEggTooltip : PetTooltip
    {
        ...
        public override string SimpleTooltip => Compatibility.LocVal("SimpleTooltips.ForgottenDragonEgg").Replace("<keybind>", PetUtils.KeybindText(PetKeybinds.UsePetAbility));
    }
}
