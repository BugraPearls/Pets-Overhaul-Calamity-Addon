...
namespace PetsOverhaulCalamityAddon.CalamityPets
{
    public sealed class DannyDevitoEffect : PetEffect
    {
        ...
        public override void ExtraProcessTriggers(TriggersSet triggersSet)
        {
            if (Pet.AbilityPressCheck() && PetIsEquipped())
            {
                ...
                Projectile petProjectile = Projectile.NewProjectileDirect(PetModPlayer.GetSource_Pet(EntitySourcePetIDs.PetProjectile), Player.Center, new Vector2(Main.MouseWorld.X - Player.Center.X, Main.MouseWorld.Y - Player.Center.Y) * 0.03f, ModContent.ProjectileType<Trashcan>(), Pet.PetDamage(damage, DamageClass.Generic), 4f, Player.whoAmI);
    ...
    public sealed class TrashmanTrashcanTooltip : PetTooltip
    {
        ...
        public override string SimpleTooltip => Compatibility.LocVal("SimpleTooltips.TrashmanTrashcan").Replace("<keybind>", PetUtils.KeybindText(PetKeybinds.UsePetAbility));
    }
}
