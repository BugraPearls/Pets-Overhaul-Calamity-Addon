...
namespace PetsOverhaulCalamityAddon.CalamityPets
{
    public sealed class ElectricTroublemakerEffect : PetEffect
    {
        ...
        public string CurrentMoveName
        {
            get
            {
                return currentMove switch
                {
                    thunderWave => Compatibility.LocVal("PetTooltips.RotomThunderWaveName"),
                    hydroPump => Compatibility.LocVal("PetTooltips.RotomHydroPumpName"),
                    overheat => Compatibility.LocVal("PetTooltips.RotomOverheatName"),
                    blizzard => Compatibility.LocVal("PetTooltips.RotomBlizzardName"),
                    leafStorm => Compatibility.LocVal("PetTooltips.RotomLeafStormName"),
                    airSlash => Compatibility.LocVal("PetTooltips.RotomAirSlashName"),
                    _ => "Invalid Class",
                };
            }
        }
            ...
            if (PetIsEquipped() && chargeNextAttack)
            {
                switch (currentMove)
                {
                    ...
                    case hydroPump:
                        ...
                        {
                            Pet.PetStrike(target, hydroPumpDmg * GetTypeEffectiveness(target, hydroPump), hit.HitDirection, Main.rand.NextBool((int)Math.Min(Player.GetTotalCritChance(hit.DamageType), 100)), 0, hit.DamageType);
                            for (int i = 0; i < 10; i++)
                            {
                    ...
                    case overheat:
                        if (Pet.timer + Pet.timerMax * overheatCooldown < Pet.timerMax)
                        {
                            Pet.NewPetSourcedProjectile(PetUtils.GetSource_Pet(EntitySourcePetIDs.PetProjectile), target.Center, Vector2.Zero, ModContent.ProjectileType<PetExplosion>(), overheatDmg * GetTypeEffectiveness(target, overheat), 0, Player.whoAmI, overheatRadius, damageClass: hit.DamageType);
                            if (target.active)
                            {
                            ...
                            Pet.NewPetSourcedProjectile(PetUtils.GetSource_Pet(EntitySourcePetIDs.PetProjectile), Player.Center, Vector2.Zero, ModContent.ProjectileType<PetExplosion>(), overheatDmg * GetTypeEffectiveness(target, overheat), 0, Player.whoAmI, overheatRadius, damageClass: hit.DamageType);
                            for (int i = 0; i < 10; i++)
                            {
                    ...
                    case leafStorm:
                        if (Pet.timer + Pet.timerMax * leafStormCooldown < Pet.timerMax)
                        {
                            ...
                            {
                                Pet.NewPetSourcedProjectile(PetUtils.GetSource_Pet(EntitySourcePetIDs.PetProjectile), target.Center + Main.rand.NextVector2CircularEdge(target.width, target.height), Main.rand.NextVector2CircularEdge(10, 10), ProjectileID.Leaf, leafStormDmg * GetTypeEffectiveness(target, leafStorm), 0, Player.whoAmI, damageClass: hit.DamageType);
                            }
                            if (ModContent.GetInstance<PetPersonalization>().AbilitySoundEnabled)
                    ...
                    case airSlash:
                        if (Pet.timer + Pet.timerMax * airSlashCooldown < Pet.timerMax)
                        {
                            foreach (var npc in Main.ActiveNPCs)
                            {
                                ...
                                {
                                    Pet.PetStrike(npc, airSlashDmg * GetTypeEffectiveness(npc, airSlash), hit.HitDirection, Main.rand.NextBool((int)Math.Min(Player.GetTotalCritChance(hit.DamageType), 100), 100), airSlashKb * GetTypeEffectiveness(npc, airSlash), damageType: hit.DamageType);
                                }
                            }
                            for (int i = 0; i < 15; i++)
                            {
    ...
    public sealed class TheEtomerTooltip : PetTooltip
    {
        public override PetEffect PetsEffect => rotom;
        public static ElectricTroublemakerEffect rotom
        {
            get
            {
                if (Main.LocalPlayer.TryGetModPlayer(out ElectricTroublemakerEffect pet))
                    return pet;
                else
                    return ModContent.GetInstance<ElectricTroublemakerEffect>();
            }
        }
            ...
            get
            {
                ...
                switch (rotom.currentMove)
                {
                    case ElectricTroublemakerEffect.thunderWave:
                        MoveTooltip = Compatibility.LocVal("PetTooltips.RotomCurrent") + rotom.CurrentMoveName + "\n" +
                            Compatibility.LocVal("PetTooltips.RotomThunderWave")
                    ...
                    case ElectricTroublemakerEffect.hydroPump:
                        MoveTooltip = Compatibility.LocVal("PetTooltips.RotomCurrent") + rotom.CurrentMoveName + "\n" +
                            Compatibility.LocVal("PetTooltips.RotomHydroPump")
                    ...
                    case ElectricTroublemakerEffect.overheat:
                        MoveTooltip = Compatibility.LocVal("PetTooltips.RotomCurrent") + rotom.CurrentMoveName + "\n" +
                            Compatibility.LocVal("PetTooltips.RotomOverheat")
                    ...
                    case ElectricTroublemakerEffect.blizzard:
                        MoveTooltip = Compatibility.LocVal("PetTooltips.RotomCurrent") + rotom.CurrentMoveName + "\n" +
                            Compatibility.LocVal("PetTooltips.RotomBlizzard")
                    ...
                    case ElectricTroublemakerEffect.leafStorm:
                        MoveTooltip = Compatibility.LocVal("PetTooltips.RotomCurrent") + rotom.CurrentMoveName + "\n" +
                            Compatibility.LocVal("PetTooltips.RotomLeafStorm")
                    ...
                    case ElectricTroublemakerEffect.airSlash:
                        MoveTooltip = Compatibility.LocVal("PetTooltips.RotomCurrent") + rotom.CurrentMoveName + "\n" +
                            Compatibility.LocVal("PetTooltips.RotomAirSlash")
                ...
                return Compatibility.LocVal("PetTooltips.TheEtomer")
                       .Replace("<abilitySwitch>", PetUtils.KeybindText(PetKeybinds.PetAbilitySwitch))
                       .Replace("<abilityUse>", PetUtils.KeybindText(PetKeybinds.UsePetAbility))
        ...
        public override string SimpleTooltip => Compatibility.LocVal("SimpleTooltips.TheEtomer")
            .Replace("<abilitySwitch>", PetUtils.KeybindText(PetKeybinds.PetAbilitySwitch)).Replace("<abilityUse>", PetUtils.KeybindText(PetKeybinds.UsePetAbility));
    }
}
